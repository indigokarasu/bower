...
During `bower.scan.deep`, Bower builds a preference profile from evidence in the existing structure. Written to `~/openclaw/data/ocas-bower/preference_profile.json` and updated on every deep scan. Human-readable; the user may edit it directly. Any field marked `locked: true` is treated as immutable and never overwritten by inference.
...
Every 500 files processed, write a checkpoint to `~/openclaw/data/ocas-bower/staging/scan_checkpoint.json`. See `analysis_schema.md` for the checkpoint schema.
...
After every `bower.apply` run, produce a human-readable digest summarizing what was done. The digest is printed to the user and appended to `~/openclaw/data/ocas-bower/reports/` as a dated Markdown file.
...
- No writes to any file under `~/openclaw/`.
- No journal written.
