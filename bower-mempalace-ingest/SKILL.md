...
- **KG facts**: automatically generates `owner → visited → X`, `owner → has_domain → X`
...
3. **KG facts for queryability** — domains, places visited, medical providers as graph facts enable `mcp_mempalace_mempalace_kg_query("owner")`.
