...
- **Evidence**: Every scheduled run writes an evidence record to `{agent_root}/commons/data/ocas-bower/evidence.jsonl`, including no-op runs. The `not_activity_reason` field is mandatory when no side effects occur.
- **Gap detection**: On every wake, the skill checks the evidence log for the most recent completed run. If the gap exceeds the expected cadence (24h for light scan, 7d for deep scan), it logs `gap_detected` and runs a compact remedial pass.
- **Degraded mode**: When Google Drive access fails, the skill enters degraded mode and produces a partial Drive health report rather than skipping silently. The evidence log records `degraded: google_drive`.
- **Log compaction**: Evidence and decision logs older than 30 days (no-op) or 90 days (error/gap) are compacted to weekly summaries. Escalation records are never auto-deleted. The last 7 days of raw entries are always retained.
...
`bower.update` pulls the latest package from the `source:` URL in this file's frontmatter. Runs silently — no output unless the version changed or an error occurred.
...
7. Output exactly: `I updated Bower from version {old} to {new}`
